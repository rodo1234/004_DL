...
class TradingStrategyOptimizer_RNN:
    ...
    def objective(self, trial, X_train, y_train, X_val, y_val):
        ...
        model.add(tf.keras.layers.SimpleRNN(n_units, input_shape=( 1 , X_train.shape[2]), activation='relu'))
    ...
    def run_rnn(self):
        ...
        best_params_buy = self.optimize_rnn(X_train_buy, y_train_buy, X_val_buy, y_val_buy)
        best_params_sell = self.optimize_rnn(X_train_sell, y_train_sell, X_val_sell, y_val_sell)

        model_buy_rnn = self.build_and_train_model(best_params_buy, X_train_buy, y_train_buy)
        model_sell_rnn = self.build_and_train_model(best_params_sell, X_train_sell, y_train_sell)
        ...
        y_pred_buy = model_buy_rnn.predict(X_reshaped)
        y_pred_sell = model_sell_rnn.predict(X_reshaped)
        
        #Guardar el modelo
        model_buy_rnn.save('model_buy_rnn.keras')
        model_sell_rnn.save('model_sell_rnn.keras')

        # covertir a booleanos
        y_pred_buy = y_pred_buy > 0.5
        y_pred_sell = y_pred_sell > 0.5

        return pd.DataFrame({'Y_BUY_PRED_RNN': y_pred_buy.flatten(), 'Y_SELL_PRED_RNN': y_pred_sell.flatten()}), model_buy_rnn, model_sell_rnn
